pipeline {
    ...
    stages {
        ...
        stage('Deploy to Nexus') {
            steps {
                dir('backend') {
                    sh 'mvn deploy -DaltDeploymentRepository=nexus::default::$NEXUS_URL -DskipTests=true -Dnexus.username=$NEXUS_CREDENTIALS_USR -Dnexus.password=$NEXUS_CREDENTIALS_PSW'
                }
            }
        }
